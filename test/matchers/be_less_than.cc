...
describe("be_less_than", []{
    describe("when the actual value is a built-in type", [&]{
		...
		it("must reject a positive match", [&]{
		    [&]{ actualValue must be_less_than(expectedValue); } must throw_exception;
		});
            });
		...
		it("must reject a negative match", [&]{
		    [&]{ actualValue must_not be_less_than(expectedValue); } must throw_exception;
		});
            });
