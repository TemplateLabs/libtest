...
describe("be_lte", [&]{
    ...
    describe("when the actual value is a built-in type", [&]{
		...
		it("must reject a positive match", [&]{
		    [&]{ actualValue must be_lte(expectedValue); } must throw_exception;
		    [&]{ actualValue must be_less_than_or_equal_to(expectedValue); } must throw_exception;
		});
            });
		...
		it("must reject a negative match", [&]{
		    [&]{ actualValue must_not be_lte(expectedValue); } must throw_exception;
		    [&]{ actualValue must_not be_less_than_or_equal_to(expectedValue); } must throw_exception;
		});
            });
